...
class Seq2SeqModel:
    ...
    def model_inputs(self):
        """
        Creates uninitialized containers
            inputs : for input data
            targets : output target
            learning_rate : the rate at which the weights are adjusted so that a minimum of a loss function is reached
            keep_prob : the probability that a neuron is active (improves overfitting)
        """
    ...
    def decoder_rnn(self):
        ...
        decoder_embeddings_matrix = tf.Variable(
            tf.random_uniform([len(self.data.questions_words2int) + 1, self.params['decoding_embedding_size']], 0, 1))
